...
> **먼저 읽을 것.** 이 연결은 **되지 않는다.** 2026-09-25 에 한 시간 넘게 붙들고 쿠키 두 종류와
> 인증 해제까지 시도했으나 모두 실패했다. 원인은 강좌와 실제 인스턴스가 어긋난 것이고 우리 쪽에서
> 고칠 수 없다. **같은 시도를 되풀이하지 말고 아래 "대신 이렇게 한다" 로 바로 가라.**
> 모듈 3 과 4 의 실습이 전부 이 연결에 걸려 있어 진도는 50% 에서 멈춘다.

**시도한 것과 결과**

| 시도 | 결과 |
|---|---|
| Access session 을 비우고 연결 | 네 점검 모두 실패 |
| `cf_clearance` 쿠키 값 입력 | 실패. 이건 봇 검사 통과 표시라 인증 토큰이 아니다 |
| `__Host-skybridge-brev-prd` 쿠키 값 입력 | 실패. `__Host-` 쿠키는 규격상 교차 출처로 못 보낸다 |
| launchable 탭을 열어 둔 채로 재시도 | 실패 |
| `Disable authentication` 으로 공개 전환 | **네 점검 중 하나만 통과.** 나머지 셋은 여전히 실패 |
...
2026-09-25 시도에서는 확인 직후 인증을 다시 켜고 인스턴스를 정지했다. 공개 상태는 몇 분에 그쳤다.
...
**본문 입력은 문제가 없었다.** 작성 중에 화면을 PDF 로 인쇄해 확인했더니 본문이 잘려 보였으나,
텍스트 상자가 스크롤된 상태로 찍혀 보이는 부분만 인쇄된 것이었다. 실제 입력값은 온전했다.
같은 방식으로 확인할 때는 상자 안을 직접 훑어 보는 편이 정확하다.
