...
규제도 같은 자리를 보고 있다. FDA와 EMA는 2026년 1월 14일에 의약품 전 주기의 AI 활용 원칙
열 가지를 공동으로 냈고 안전성 모니터링이 그 범위에 들어간다. 모델이 낸 판단을 무엇으로
뒷받침할 것인가가 따라오는 질문이다.
...
열세 가지가 결합 근거를 다루고 두 가지가 사람 근거를 다룬다. 지금 검증이 두터운 쪽은 결합이다. 출처는 FDDD 데모의 `notes` 배열, NVIDIA DiffDock 문서, 기존 약물감시 규율 셋이다.
한 가지는 우리가 직접 재서 넣었다. DiffDock 호스팅 API에는 시드가 없다. 같은 수용체와 리간드로
두 번 불러 3순위 포즈 신뢰도가 0.725와 0.515로 갈렸다.
